...
registerTopic({
  ...
  desc: '拖动导体棒在磁场中切割磁感线，观察感应电动势和感应电流如何产生。',
  render(root) {
    const { canvasBox, panel } = topicPage(root, {
      title: '电磁感应（导轨模型）', en: 'Electromagnetic Induction',
      tagline: '导体棒向右滑动，回路面积增大、磁通量增加 —— 感应电流随之出现。这就是发电机的原理。',
      formula: 'ε = BLv　·　ε = <span class="frac"><span>ΔΦ</span><span class="den">Δt</span></span>（法拉第定律）　·　I = <span class="frac"><span>ε</span><span class="den">R</span></span>',
      explainHTML: `
    ...
    const sB = addSlider(panel, { label: '磁感应强度 B', en: 'field', min: 0.2, max: 2, step: 0.05, value: 0.8, unit: 'T' });
    const sL = addSlider(panel, { label: '导轨间距 L', en: 'rail spacing', min: 0.5, max: 2, step: 0.1, value: 1, unit: 'm' });
    const sv = addSlider(panel, { label: '棒的速度 v', en: 'rod speed', min: 0, max: 5, step: 0.1, value: 2.5, unit: 'm/s' });
    const sR = addSlider(panel, { label: '回路电阻 R', en: 'resistance', min: 0.5, max: 10, step: 0.5, value: 2, unit: 'Ω' });
    ...
    function draw() {
      ...
      ctx.fillText('B ⊗（向里）= ' + fmtN(B, 2) + ' T', left, topY - 28);
      ctx.restore();
      // 回路面积着色（磁通量）
      ...
      ctx.fillText('导体棒 L = ' + fmtN(L, 1) + ' m', rodPx - 40, botY + 24);
      // 速度箭头 & 安培力箭头
      if (v > 0.05) {
        pxArrow(ctx, rodPx, topY - 18, rodPx + 30 + v * 9, topY - 18, { color: C.green, width: 3, label: 'v', labelDy: 4 });
        pxArrow(ctx, rodPx, midY, rodPx - 24 - F * 16, midY, { color: C.red, width: 3, label: 'F安(阻碍)', labelDx: -86, labelDy: -8 });
      }
      // 感应电流方向（逆时针）动画箭头
      if (I > 0.01) {
        ...
        ctx.fillText('感应电流 I（逆时针）', (left + rodPx) / 2 - 50, botY + 24);
      }
