...
registerTopic({
  ...
  desc: '调节半径和角速度，观察线速度（切向）与向心加速度（指向圆心）的方向关系。',
  render(root) {
    const { canvasBox, panel } = topicPage(root, {
      title: '匀速圆周运动', en: 'Uniform Circular Motion',
      tagline: '速度沿切线、加速度指向圆心 —— "匀速"圆周运动其实一直在加速！',
      formula: 'v = ωr　·　a = <span class="frac"><span>v²</span><span class="den">r</span></span> = ω²r　·　F<sub>向</sub> = <span class="frac"><span>mv²</span><span class="den">r</span></span>　·　T = <span class="frac"><span>2π</span><span class="den">ω</span></span>',
      explainHTML: `
    ...
    const sr = addSlider(panel, { label: '半径 r', en: 'radius', min: 0.5, max: 3, step: 0.1, value: 2, unit: 'm' });
    const sw = addSlider(panel, { label: '角速度 ω', en: 'angular velocity', min: 0.3, max: 4, step: 0.1, value: 1.5, unit: 'rad/s' });
    const sm = addSlider(panel, { label: '质量 m', en: 'mass', min: 0.5, max: 5, step: 0.5, value: 1, unit: 'kg' });
    ...
    function draw() {
      ...
      plot.arrow(P[0], P[1], P[0] + tx * v * vScale, P[1] + ty * v * vScale, { color: C.green, width: 3, label: 'v（切线）', labelDy: -8 });
      const aScale = 0.65 / Math.max(1, w * w);
      plot.arrow(P[0], P[1], P[0] - Math.cos(th) * a * aScale, P[1] - Math.sin(th) * a * aScale, { color: C.red, width: 3, label: 'a（指向圆心）', labelDy: 16 });
      // 小球
