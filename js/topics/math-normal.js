...
registerTopic({
  ...
  desc: '调整均值 μ 和标准差 σ，观察钟形曲线的移动与胖瘦，理解 68-95-99.7 法则。',
  render(root) {
    const { canvasBox, panel } = topicPage(root, {
      title: '正态分布', en: 'Normal (Gaussian) Distribution',
      tagline: 'μ 决定钟形曲线的位置，σ 决定它的胖瘦 —— 彩色区域就是著名的 68-95-99.7 法则。',
    ...
    const smu = addSlider(panel, { label: '均值 μ', en: 'mean', min: -3, max: 3, step: 0.1, value: 0 });
    const ssig = addSlider(panel, { label: '标准差 σ', en: 'std. dev.', min: 0.4, max: 2.5, step: 0.05, value: 1 });
