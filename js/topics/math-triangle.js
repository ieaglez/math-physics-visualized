...
registerTopic({
  ...
  desc: '调整两边及其夹角，实时解出第三边和其余两角，并验证正弦定理的外接圆意义。',
  render(root) {
    const { canvasBox, panel } = topicPage(root, {
      title: '解三角形', en: 'Solving Triangles',
      tagline: '已知两边及夹角 (SAS)，正弦定理和余弦定理帮你解出整个三角形 —— 虚线圆是外接圆。',
    ...
    const sb = addSlider(panel, { label: '边 b（AC）', en: 'side', min: 2, max: 9, step: 0.1, value: 6 });
    const sc = addSlider(panel, { label: '边 c（AB）', en: 'side', min: 2, max: 9, step: 0.1, value: 8 });
    const sA = addSlider(panel, { label: '夹角 A', en: 'included angle', min: 15, max: 165, step: 1, value: 60, unit: '°' });
    ...
    function draw() {
      ...
      ctx.restore();
      plot.dot(ox, oy, { color: '#b6bbe3', r: 3, label: 'O（外心）', labelDx: 8 });
      // 三角形
      ctx.save();
      ...
      const angleType = A > Math.PI / 2 + 0.001 ? '钝角三角形 (obtuse)' : Math.abs(A - Math.PI / 2) < 0.01 ? '直角三角形 (right)！a² = b² + c²' : (B > Math.PI/2 || Cc > Math.PI/2) ? '钝角三角形 (obtuse)' : '锐角三角形 (acute)';
      readout.set(`
