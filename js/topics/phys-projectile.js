...
registerTopic({
  ...
  desc: '调整初速度、抛射角和重力加速度，发射！观察抛物线轨迹与速度的分解。',
  render(root) {
    const { canvasBox, panel } = topicPage(root, {
      title: '抛体运动', en: 'Projectile Motion',
      tagline: '调好角度和初速度后点击播放发射，注意小球上的速度分量箭头。',
      formula: 'x = v₀cosθ·t　y = v₀sinθ·t − ½gt<sup>2</sup>　·　射程 R = <span class="frac"><span>v₀²sin2θ</span><span class="den">g</span></span>',
      explainHTML: `
    ...
    const sv0 = addSlider(panel, { label: '初速度 v₀', en: 'initial speed', min: 5, max: 30, step: 0.5, value: 20, unit: 'm/s' });
    const sth = addSlider(panel, { label: '抛射角 θ', en: 'launch angle', min: 5, max: 85, step: 1, value: 45, unit: '°' });
    const sg = addSlider(panel, { label: '重力加速度 g', en: 'gravity', min: 1.6, max: 20, step: 0.1, value: 9.8, unit: 'm/s²' });
    ...
    function draw() {
      ...
      ctx.restore();
      // 关键点标注
      plot.dot(R / 2, Hm, { color: C.gray, r: 3.5, label: `最高点 H=${fmtN(Hm,1)}m`, labelDy: -8, labelDx: -30 });
      plot.dot(R, 0, { color: C.gray, r: 3.5, label: `射程 R=${fmtN(R,1)}m`, labelDy: -10, labelDx: -40 });
      // 当前小球与速度分量
