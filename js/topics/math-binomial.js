...
registerTopic({
  ...
  desc: '调整试验次数 n 和单次成功率 p，看概率分布的形状，并预览它如何逼近正态分布。',
  render(root) {
    const { canvasBox, panel } = topicPage(root, {
      title: '二项分布', en: 'Binomial Distribution B(n, p)',
      tagline: '投 n 次篮、命中 k 次的概率分布。把 n 调大 —— 柱子的轮廓越来越像一条钟形曲线。',
    ...
    const sn = addSlider(panel, { label: '试验次数 n', en: 'trials', min: 2, max: 30, step: 1, value: 10 });
    const sp = addSlider(panel, { label: '单次成功概率 p', en: 'probability', min: 0.05, max: 0.95, step: 0.05, value: 0.5 });
    ...
    function draw() {
      ...
      plot.clear('#fff');
      const { ctx } = cv;
      plot.axes({ xLabel: 'k（成功次数）', yLabel: 'P', tickX: n > 15 ? 5 : 1, tickY: 1e9 });
      ...
      if (n >= 8) {
        ...
        plot.text(mu + 1.6 * sig, pmax * 0.75, '正态近似 N(np, npq)', { color: C.orange, font: 'bold 12px sans-serif' });
      }
      // 期望线
