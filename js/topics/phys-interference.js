...
registerTopic({
  ...
  desc: '改变波长（颜色）、缝距和屏距，观察明暗条纹如何变宽变窄 —— 光是波的铁证。',
  render(root) {
    const { canvasBox, panel } = topicPage(root, {
      title: '光的双缝干涉', en: "Young's Double-Slit Interference",
      tagline: '右侧竖条是屏上的干涉条纹。换个颜色（波长），条纹间距立刻改变。',
      formula: '相邻亮纹间距 Δy = <span class="frac"><span>λL</span><span class="den">d</span></span>　·　亮纹条件：路程差 δ = kλ',
      explainHTML: `
    ...
    const sl = addSlider(panel, { label: '波长 λ（决定颜色）', en: 'wavelength', min: 400, max: 700, step: 5, value: 550, unit: 'nm' });
    const sd = addSlider(panel, { label: '双缝间距 d', en: 'slit separation', min: 0.1, max: 0.8, step: 0.02, value: 0.3, unit: 'mm' });
    const sL = addSlider(panel, { label: '屏距 L', en: 'screen distance', min: 0.5, max: 3, step: 0.1, value: 1.5, unit: 'm' });
    ...
    function draw() {
      ...
      ctx.fillText('屏上的干涉条纹 →', screenX - 130, 44);
      ctx.fillText('← 光强分布 I', screenX - 130, 62);
      const visible = ['紫', '蓝', '青', '绿', '黄', '橙', '红'][Math.min(6, Math.floor((lam - 400) / 43))];
      readout.set(`
        波长 λ = <b>${fmtN(lam,0)} nm</b>（${visible}光）<br>
